...
public final class EmbeddedLauncher {
    ...
    private static final String TAG = "EmbeddedLauncher";
    ...
    static class PublisherListener extends AbstractInterceptHandler {
        ...
        public void onPublish(InterceptPublishMessage msg) {
            byte[] body = new byte[msg.getPayload().readableBytes()];
            msg.getPayload().getBytes(0, body);
            final String decodedPayload = new String(body, UTF_8);
            Log.d(TAG, "Received on topic: " + msg.getTopicName() + " content: " + decodedPayload);
        }
    }
    ...
    public static void main() throws InterruptedException, IOException {
        ...
        Log.d(TAG, "Broker started press [CTRL+C] to stop");
        //Bind  a shutdown hook
        Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d(TAG, "Stopping broker");
                mqttBroker.stopServer();
                Log.d(TAG, "Broker stopped");
            }
        }));

        Thread.sleep(20000);
        Log.d(TAG, "Before self publish");
